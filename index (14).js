...
client.on("messageCreate", async (message) => {
  // Ignore other bots (and Bob himself)
  if (message.author.bot) return;

  const isDM = message.guildId === null;
  const mentioned = client.user && message.mentions.users.has(client.user.id);

  // Also trigger on a plain-text "bob" at the START of the message,
  // e.g. "bob send the 4k link" - no blue @mention needed.
  const startsWithBob = /^\s*bob\b[\s,:!-]*/i.test(message.content);

  // In servers, reply if Bob is @mentioned OR the message starts with "bob".
  // In DMs, always reply.
  if (!isDM && !mentioned && !startsWithBob) return;

  // Strip the @mention and/or the leading "bob" so Bob just sees the real question.
  const userMessage = message.content
    .replace(/<@!?\d+>/g, "")
    .replace(/^\s*bob\b[\s,:!-]*/i, "")
    .trim();
  const threadKey = isDM
    ? `dm-${message.channelId}-${message.author.id}`
    : message.channelId;

  if (!userMessage) {
    await message.reply("Hey! I'm Bob. Ask me about payment links, flipping questions, key links - whatever you need.");
    return;
  }
  ...
  try {
    ...
    await message.reply(`Error: ${err?.message || "Something went wrong."}`);
  }
});

client.once("ready", () => {
  console.log(`Bob is online and ready as ${client.user.tag}.`);
});
